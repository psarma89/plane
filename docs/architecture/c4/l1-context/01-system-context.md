...
That server lives in a separate repository, so the numbers above carry a pin. Measured at `makeplane/plane-mcp-server` commit `96cf4d5`, dated 2026-07-22. Re-derive the tool count with `grep -rc '@mcp.tool(' plane_mcp/tools/`, and the module count with `grep -rl '@mcp.tool(' plane_mcp/tools/`. Read the four transports from the mounts in `plane_mcp/__main__.py`.
